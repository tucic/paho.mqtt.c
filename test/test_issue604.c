...
void getopts(int argc, char** argv)
{
    ...
    while (count < argc)
    {
        ...
        else if (strcmp(argv[count], "--trust_store") == 0)
        {
            if (++count < argc)
                options.trust_store = argv[count];
            else
                usage();
        }
...
Test 1: connect on connectionLost

*********************************************************************/
MQTTClient test1_c1;
volatile int test1_connection_lost_called = 0;
volatile int test1_connection_lost_called_once = 0;

void test1_connectionLost(void* context, char* cause)
{
    printf("Callback: connection lost\n");
    test1_connection_lost_called = 1;
    test1_connection_lost_called_once = 1;
}
...
char* test1_will_topic = "C Test 1: will topic";
char* test1_will_message = "will message from Client-1";
...
int test1_messageArrived(void* context, char* topicName, int topicLen, MQTTClient_message* m)
{
    ...
    printf("%s -> Callback: message received on topic '%s' is '%.*s'.\n",
             (c == test1_c1) ? "Client-1" : "Client-2", topicName, m->payloadlen, (char*)(m->payload));
...
int test1(struct Options options)
{
    ...
    failures = 0;
    MyLog(LOGA_INFO, "Starting test 1 - reconnect on connectionLost");
    ...
    {
        opts.ssl = &sslopts;
        opts.ssl->trustStore = options.trust_store;
    ...
    MyLog(LOGA_INFO, "Waiting connection lost (shutdown broker)");
    ...
    while (retries < 500)
    {
        ...
        if (test1_connection_lost_called == 1) {
            MyLog(LOGA_INFO, "Sleeping %d s ...", options.reconnect_period);
            mysleep(options.reconnect_period);

            if (retries==0) {
                MyLog(LOGA_INFO, "Trying first re-connect (broker should be down) ...", ++retries);
                rc = MQTTClient_connect(test1_c1, &opts);
                assert("broker should be down, rc should indicate failure",  rc != MQTTCLIENT_SUCCESS, "rc was %d\n", rc);
            } else {
                MyLog(LOGA_INFO, "Trying to connect (restart broker) (%d) ...", ++retries);
                rc = MQTTClient_connect(test1_c1, &opts);
            }

            if(rc==0) {
                test1_connection_lost_called = 0;
                retries = 0;
                MyLog(LOGA_INFO, "Connected.");
                break;
            }else{
                MyLog(LOGA_INFO, "Connect failed, rc=%d", rc);
            }
        }
    }

    assert("connection lost called once", test1_connection_lost_called_once == 1,
                     "connection_lost_called_once %d\n", test1_connection_lost_called_once);

    rc = MQTTClient_isConnected(test1_c1);
    assert("Client not connected", rc == 1, "isconnected is %d", rc);
